...
sub writeOutput {
	...
	print OUTFILE "import GLKit \n\nlet ".$object."NumVerts:Int = ".($numFaces * 3)."\n\n";

	# write verts
	print OUTFILE "let ".$object."Verts:[GLfloat] = [\n";
	for( $j = 0; $j < $numFaces; $j++)
	{
		$ia = fixedIndex($va_idx[$j], $numVerts);
		$ib = fixedIndex($vb_idx[$j], $numVerts);
		$ic = fixedIndex($vc_idx[$j], $numVerts);
		$id = fixedIndex($na_idx[$j], $numNormals);
		$ie = fixedIndex($nb_idx[$j], $numNormals);
		$if = fixedIndex($nc_idx[$j], $numNormals);
		print OUTFILE "  // $face_line[$j]\n";

		print OUTFILE "  $xcoords[$ia], $ycoords[$ia], $zcoords[$ia],";
		print OUTFILE "  $nx[$id], $ny[$id], $nz[$id], 1.0, 1.0,\n";

		print OUTFILE "  $xcoords[$ib], $ycoords[$ib], $zcoords[$ib],";
		print OUTFILE "  $nx[$ie], $ny[$ie], $nz[$ie], -1.0, 1.0,\n";

		print OUTFILE "  $xcoords[$ic], $ycoords[$ic], $zcoords[$ic],";
		print OUTFILE "  $nx[$if], $ny[$if], $nz[$if], 1.0, -1.0,\n";
	}
	print OUTFILE "]\n\n";
	...
	# if($numTexture) {
	# 	print OUTFILE "let ".$object."TexCoords = [\n";
	# 	for( $j = 0; $j < $numFaces; $j++) {
